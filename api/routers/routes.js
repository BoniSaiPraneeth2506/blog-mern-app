...
// const fs = require('fs');
// const { router, post } = require('../server');
...
route.post('/login', async (req, res) => {
  ...
  try {
    ...
    const passOk = await bcrypt.compare(passWord, userDoc.passWord);
    if (passOk) {
      const token = jwt.sign({ userId: userDoc._id, userName: userDoc.userName }, process.env.JWT_SECRET, { expiresIn: '1h' });
...
route.put('/edit/:id', uploadMiddleware.single('file'), async (req, res) => {
  try {
    ...
    jwt.verify(token, process.env.JWT_SECRET, {}, async (err, info) => {
      ...
      if (!post) return res.status(404).json({ message: 'Post not found' });
      if (post.author.toString() !== info.userId) return res.status(403).json({ message: 'Unauthorized' });
...
route.delete('/delete/:id', async (req, res) => {
  try {
    ...
    jwt.verify(token, process.env.JWT_SECRET, {}, async (err, info) => {
      ...
      if (post.cover) {
        const filePath = path.join(__dirname, '..', 'uploads', post.cover);
...
route.get('/myposts', async (req, res) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET, {}, async (err, info) => {
    ...
    const posts = await Post.find({ author: info.userId }).populate('author', ['userName']).sort({ createdAt: -1 });
    res.json(posts);
  });
});
