...
//     const newUser=await User.create({ userName, passWord:hashedPassword }); // 👈 simple and clean
...
//     res.json(info); // userInfo will have { userId, userName, ... }
//   });
// });
...
// route.post('/create', uploadMiddleware.single('file'), async (req, res) => {
//   const { originalname, path: tempPath } = req.file; // ✅ avoid overwriting `path` module
//   const parts = originalname.split('.');
//   const ext = parts[parts.length - 1];
...
//       summary,
//       content,
//       cover: `/uploads/${fileName}`, // ✅ browser-accessible path
//       author: info.userId
//     });
